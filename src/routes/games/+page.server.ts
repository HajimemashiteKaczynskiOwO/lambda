...
export const actions = {
  match: async ({ request, cookies }) => {
    ...
    if (!player1Id || !player2Id || !winnerChoice) return fail(400, { error: 'All fields required' });
    if (player1Id === player2Id) return fail(400, { error: 'Players must be different' });

    const player1 = await prisma.player.findUnique({ where: { id: player1Id }, select: { username: true } });
    const player2 = await prisma.player.findUnique({ where: { id: player2Id }, select: { username: true } });
    ...
    const winner = await prisma.player.findUnique({ where: { id: winnerId }, select: { wins: true, matches: true } });
    ...
    const loser = await prisma.player.findUnique({ where: { id: loserId }, select: { wins: true, matches: true } });
    await prisma.player.update({
      where: { id: loserId },
      data: {
        losses: { increment: 1 },
